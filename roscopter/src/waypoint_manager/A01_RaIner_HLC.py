...
class hl_cmd_handler(object):
    def __init__(self):
        ...
        self.takeoff_height_agl = self.takeoff_cmd.F
    ...
    def rc_cb(self, data):
        ...
        if((self.rc_msg.values[self.rc_mode_chn] <= 1700) and (self.rc_msg.values[self.rc_mode_chn] > 1400)):
            ...
            else if(self.takeoff_complete and self.state == 'nearness_control')
                self.state = 'pos_hold'
    ...
    def odom_cb(self, data):
        ...
        self.yaw = self.invert_odom*yaw_angle*self.yaw

        if(self.state == 'init' && not self.odom_initialized):
            self.initial_odom_state = self.odom_msg
        ...
        while not rospy.is_shutdown():
            if(self.offb_switch):
                ...
                if(self.state == 'landing'):
                    ...
                    if(self.pos_z < self.landing_thresh):
                        self.state = 'landed'
