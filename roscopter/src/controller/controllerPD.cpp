                                      ...
                                      uint32_t level)
{
  ...
  PID_xtot_.setGains(Kpx, 0, 0, tau);//bnr - set values for PD controller
  PID_ytot_.setGains(Kpy, 0, 0, tau);
...
void Controller::computeControl(double dt)
{
 ...
 //bnr   xc_.ax = PID_x_dot_.computePID(xc_.x_dot, pxdot, dt);
 //bnr   xc_.ay = PID_y_dot_.computePID(xc_.y_dot, pydot, dt);

    // Nested Loop for Altitude
    double pddot_c = PID_d_.computePID(xc_.pd, xhat_.pd, dt, pddot);
 //bnr   xc_.az = PID_z_dot_.computePID(pddot_c, pddot, dt);
	double xff, yff, zff;
	xff = 0;
	yff = 0;
	zff = 0;
    ...
    xc_.ax = PID_xtot_.computePID(xc_.pn, xhat_.pn, dt)+PID_xveltot_.computePID(xc_.x_dot, pxdot, dt) + 0*xff;
    xc_.ay = PID_ytot_.computePID(xc_.pe, xhat_.pe, dt)+PID_yveltot_.computePID(xc_.y_dot, pydot, dt) + 0*yff;
    xc_.az = PID_ztot_.computePID(xc_.pd, xhat_.pd, dt)+PID_zveltot_.computePID(pddot_c, pddot, dt) + 0*zff;	
